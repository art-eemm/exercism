...
export function fixListOfWagons(ids) {
  ...
  const fixedList = [...rest, first, second]
  return fixedList
}
...
export function correctListOfWagons(ids, missingWagons) {
  ...
  const fixedList = [first, ...missingWagons, ...rest]
  return fixedList
}
...
export function extendRouteInformation(information, additional) {
  ...
  const {timeOfArrival, ...rest} = additional
  const result = {from, ...rest, timeOfArrival, to}
  return result
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, to, ...rest} = information
  const arr = [timeOfArrival, {...rest, to}]
  return arr
}
